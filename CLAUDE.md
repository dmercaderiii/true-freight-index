...
- [lib/database-records.ts](lib/database-records.ts) — validates and converts output rows (the `OutputRow` tuple) into `DatabaseRateRecord` objects for insertion (numeric checks, date format checks, row-count cap at 50,000).
  ...
  - `partitionNewRecords` compares the payload against the stored rows only. It does not dedupe the payload against itself: a workbook routinely lists the same lane twice at two different rates, and since `IDENTITY_COLUMNS` excludes the rate, collapsing those repeats silently drops real quotes. Do not reintroduce in-payload deduplication.
- [app/api/database-upload/route.ts](app/api/database-upload/route.ts) — server-only Next.js route handler (`runtime = "nodejs"`). Checks a shared passcode (`DATABASE_UPLOAD_PASSWORD`) with a timing-safe comparison, then selects the stored identities for the payload's effective dates, filters out duplicates, and inserts batches of 500 rows into `public.cea_to_nae_naw_rate_analysis_report` via the `postgres` package inside a transaction.
  - `mode: "check"` in the request body runs the same comparison and returns `{ total, stored, fresh }` without writing; anything else inserts and returns `{ total, stored, inserted }`. The UI checks first so it can refuse an upload whose rows are all `stored` and ask for confirmation when only some are — see `checkThenAppend` in `page.tsx`.
